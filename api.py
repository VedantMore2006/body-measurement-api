...
async def detect_measurements(file: UploadFile = File(...)):
    ...
    contents = await file.read()
    img = cv2.imdecode(np.frombuffer(contents, np.uint8), cv2.IMREAD_COLOR)
    print(f"Image shape: {img.shape if img is not None else 'None'}")  # Debug
    ...
    for cnt in contours:
        ...
        if 1.3 < aspect_ratio < 1.5 and 400 < h < 700 and y < img.shape[0] * 0.6:
            a4_height_pixels = h
            cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
            print(f"A4 detected, height: {a4_height_pixels}")  # Debug
            break

    if not a4_height_pixels:
        print("A4 not detected")  # Debug
        return {"error": "A4 not detected!"}
    ...
    for r in results:
        for box in r.boxes.xyxy:
            ...
            print(f"Person detected, height: {total_height_cm}")  # Debug
            break

    if person_bbox is None:
        print("Person not detected")  # Debug
        return {"error": "Person not detected!"}
    ...
    for r in pose_results:
        ...
        print("Returning parameters")  # Debug
        return params  # make sure this is only returned once per valid pose

    print("No pose data detected")  # Debug
    return {"error": "No pose data detected!"}
...
# async def verify_api_key(x_api_key: str = Header(None)):
#     api_key = os.getenv(
#         "API_KEY", "gY5pR3L8aB1nS9eK4dH6cJ7mF2qV4oX"
#     )  # Fallback if not set
